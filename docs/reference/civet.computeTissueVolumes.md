...
Actually, this function really returns the number of voxels of each
tissue type contained within the final discrete Civet-produced
classification volume. Now, given that Civet volumes are currently
sampled using 1-mm isotropic voxels, the voxel count value should also
reflect the volume in cubic millimeters. If this ever changes, we're
going to have to make a minor change in this function. Please let me
know if this ever happens. The native volume measurements are created by
taking the stereotactic volumes and dividing each of them by the
xfm-derived rescaling factor.
