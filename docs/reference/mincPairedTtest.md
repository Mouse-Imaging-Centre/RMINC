...
Perform a paired t-test across a set of minc volumes

## Usage

``` r
mincPairedTtest(filenames, grouping, mask = NULL, maskval = NULL)
```
...
- grouping:
  ...
  two groups with which to compare means. The two groups must be the
  same length.
