  ...
  evalq(
    {
      ...
      ref_jacobians <-
        sapply(gf$jacobians_0.2, mincGetVolume) %>%
        apply(2, function(vol) {
          tapply(vol, seg_vol, function(j) sum(exp(j) * vox_vol))
        }) %>%
        t %>%
        .[, colnames(.) %in% known_labels]
  ...
  evalq(
    {
      ...
      gold_volumes <-
        ...
        reduce(rbind)
      expect_equal(unname(unanat(label_volume)), unname(gold_volumes), ignore_attr = TRUE)

      # Test means
      label_means <-
        expect_warning(
          verboseRun(anatGetAll(
            val_files,
            seg_files,
            defs = NULL,
            method = "means"
      ...
      gold_means <-
        ...
        reduce(rbind)
      expect_equal(unname(unanat(label_means)), unname(gold_means), tolerance = 10e-5, ignore_attr = TRUE)
      ...
      gold_sums <-
        ...
        reduce(rbind)
      expect_equal(unname(unanat(label_sums)), unname(gold_sums), tolerance = 10e-5, ignore_attr = TRUE)
      ...
      gold_jacobians <-
        ...
        reduce(rbind)
      expect_equal(unname(unanat(label_jacobians)), unname(gold_jacobians), tolerance = 10e-5, ignore_attr = TRUE)
    },
    envir = test_env
  )
})
...
test_that("AnatGetAll Flags Garbage", {
  ...
  expect_error(
    anatGetAll(gf2$jacobians_0.2, defs = labels, atlas = segmentation),
...
test_that("Multires Works", {
  ...
  sprintf(
    "mincresample %s -tfm_input_sampling -transformation %s -step 1 1 1 %s",
    first_file,
    xfm,
    transformed_file
  ) %>%
  ...
  vols <-
    anatGetAll(
      c(as.character(gf$jacobians[1:5]), transformed_file),
      defs = labels,
      atlas = segmentation,
      strict = FALSE
    )

  expect_equal(unname(vols[1, ] * 1000), unname(vols[6, ]), ignore_attr = TRUE)
})

test_that("AnatGetAll errors correctly", {
  expect_error(
    anatGetAll(gf$jacobians_0.2, atlas = segmentation, method = "labels"),
...
test_that("Anatomy hierarchy works", {
  evalq(
    {
      skip_if_not(has_mincstuffs)
      hier_res <- anatSummarize(
        anat = new_jacobians,
        summarize_by = "hierarchy",
        defs = test_hier1_file
      )
  ...
  evalq(
    {
      ...
      hier_res <- anatSummarize(
        anat = new_jacobians,
        summarize_by = "other.colname",
        defs = test_hier2_file
      )
  ...
  evalq(
    {
      ...
      vol <- anatToVolume(mlm, seg, 1, defs = labels)
      ...
      corrects <-
        ...
        dplyr::mutate(
          correct = purrr::map2_lgl(
            Structure,
            label,
            ~ all(mlm[.x, 1] == vol[seg == .y])
          )
        )
