...
mm_masked <- verboseRun(
  "mincMean(gf$jacobians_fixed_2, 
                        mask = file.path(dataPath, 'testminc-mask.mnc'))",
  getOption("verbose")
)
ma_masked <- verboseRun(
  "mincApplyRCPP(gf$jacobians_fixed_2, mean, 
                        mask = file.path(dataPath, 'testminc-mask.mnc'),
                        slab_sizes = c(1,5,5))",
  getOption("verbose")
)
...
test_that("mincApplyRCPP masks like mincMean", {
  expect_equal(unname(mm_masked), unname(ma_masked))
