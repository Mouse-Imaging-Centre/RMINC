...
test_that("Weighted anatLm works", {
  ...
  expect_equal(unname(as.numeric(t(sapply(lmods, function(m) {
    ...
    })))), unname(as.numeric(alm[, grepl("tvalue", colnames(alm))])), ignore_attr = TRUE)
  expect_equal(unname(as.numeric(t(sapply(lmods, function(m) summary(m)$r.squared)))), unname(as.numeric(alm[, "R-squared"])), ignore_attr = TRUE)
  expect_equal(unname(as.numeric(t(sapply(lmods, function(m) summary(m)$fstatistic["value"])))), unname(as.numeric(alm[, "F-statistic"])), ignore_attr = TRUE)
  expect_equal(unname(as.numeric(t(sapply(lmods, coef)))), unname(as.numeric(alm[, grepl("beta", colnames(alm))])), ignore_attr = TRUE)
  expect_equal(unname(sapply(lmods, logLik)), unname(alm[, "logLik"]), ignore_attr = TRUE)
  expect_equal(unname(sapply(lmods, AIC)), unname(AIC(alm)), ignore_attr = TRUE)
})
