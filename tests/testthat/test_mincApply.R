...
ma <- verboseRun(
  "mincApply(gf$jacobians_fixed_2,quote(mean(x)))",
  getOption("verbose")
)

test_that("mincApply one output", {
  expect_equal(unname(as.numeric(mm)), unname(as.numeric(ma), ignore_attr = TRUE))
})
...
ma <- verboseRun(
  "mincApply(gf$jacobians_fixed_2, quote(testFunc(x)))",
  getOption("verbose")
)
...
ma <- verboseRun(
  "pMincApply(gf$jacobians_fixed_2, mean, 
...
ma <- verboseRun(
  "pMincApply(gf$jacobians_fixed_2, testFunc, 
                             local = TRUE, cores = min(2, parallel::detectCores() - 1))",
  getOption("verbose")
)
