  ...
  evalq(
    {
      ...
      handle_conv_warnings(
        lmer_res_nlhs <- anatLmer(
          ~ Pain.sensitivity + (1 | Genotype),
          data = gf,
          anat = jacobians
        )
      )
      ...
      handle_conv_warnings(
        lmer_res_lhs <- anatLmer(
          dummy ~ Pain.sensitivity + (1 | Genotype),
          data = gf,
          anat = jacobians
        )
      )
  ...
  evalq(
    {
      handle_conv_warnings({
        verboseRun(
          exotic_lmer <- anatLmer(
            ~ I(factor(as.numeric(Pain.sensitivity) - 1)) + (1 | Genotype),
            data = gf,
            anat = jacobians
          )
        )
