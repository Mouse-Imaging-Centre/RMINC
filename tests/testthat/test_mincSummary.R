...
strain_levels <- levels(gf_paired$Strain)
pttt <- t.test(gf_paired$vox[gf_paired$Strain == strain_levels[1]],
               gf_paired$vox[gf_paired$Strain == strain_levels[2]],
               paired = TRUE)
...
mc <- verboseRun(
  "mincCorrelation(gf$jacobians_0.2,gf$Weight)",
  getOption("verbose")
)
...
mw <- verboseRun(
  "mincWilcoxon(gf_paired$jacobians_0.2,gf_paired $Strain)",
  getOption("verbose")
)
