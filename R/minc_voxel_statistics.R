...
mincApplyRCPP <-
  function(
    ...
    if (return_indices) {
      results <- list(vals = results, inds = results_indexed$inds[result_order])
    }
    ...
    if (use_mask) {
      attr(collated_results, "mask") <- mask
    }
...
mincAnova <- function(
  ...
  if (!is.null(mask)) {
    ...
    if (
      maskDim[1] != volumeDim[1] ||
        maskDim[2] != volumeDim[2] ||
        maskDim[3] != volumeDim[3]
    ) {
      cat(paste(
        "Mask Volume",
        as.character(maskDim[1]),
        as.character(maskDim[2]),
        as.character(maskDim[3]),
        "Input Volume",
        as.character(volumeDim[1]),
        as.character(volumeDim[2]),
        as.character(volumeDim[3]),
        '\n'
      ))
  ...
  if (is.null(parallel)) {
    ...
    new_mask_file <- create_parallel_mask(
    ...
    on.exit(try({
      if (cleanup) unlink(new_mask_file)
    }))
...
mincLm <- function(
  ...
  parallel_mincLm_c <-
    function(group, plm, mask, mask_vol) {
      mincLm_c_wrapper(plm, mask, mask_min = group, mask_max = group)[
        mask_vol == group,
      ]
    }
  ...
  if (parseLmOutput$matrixFound) {
    ...
    enoughAvailableFileDescriptors(length(c(
  ...
  if (is.null(parallel)) {
    ...
    groups <- seq_len(n_groups)
    new_mask_file <- create_parallel_mask(
      sample_file = parseLmOutput$data.matrix.left[1],
      mask = mask,
      n = n_groups
    )
...
mincPairedTtest <- function(filenames, grouping, mask = NULL, maskval = NULL) {
  ...
  if (lenght_group_1 != lenght_group_2) {
    ...
    stop(
      "The groups do not have the same length. mincPairedTtest expects 2 groups with equal length (paired data)"
    )
  }
  result <- mincSummary(
    filenames,
...
mincRandomize_core <-
  function(
    ...
    boot_model <- function(n) {
      Reduce(
        function(max_val_matrix, i) {
          # Permute the filenames
          shuf_data <- lmod_data
          shuf_data[, pred_cols] <-
            lmod_data[
              sample(seq_len(nrow(lmod_data)), replace = replace),
              pred_cols
            ]
    ...
    if (is.null(parallel)) {
      ...
      if (parallel[1] %in% c("local", "snowfall")) {
        ...
        reg <- qMincRegistry(
          new_file("mincRandomize_registry"),
          conf_file = conf_file
        )
        on.exit(tenacious_remove_registry(reg), add = TRUE)

        suppressWarnings(
          #Warning suppression for large env for bootmodel (>10mb)
...
print.mincLm_randomization <-
  function(x, probs = c(.01, .05, .1, .2), ...) {
    cat(
      "mincLm_randomization results for call:\n",
      deparse(x$args$call),
      "\n",
...
print.mincTFCE_randomization <-
  function(x, probs = c(.01, .05, .1, .2), ...) {
    cat(
      "mincTFCE_randomization results for call:\n",
      deparse(x$args$call),
      "\n",
      "TFCE was run on ",
