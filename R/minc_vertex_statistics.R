...
matrixApply <- function(
  ...
  apply_fun <- function(sub_matrix) {
    ...
    lapply(seq_len(nrow(sub_matrix)), function(i) fun(sub_matrix[i, ], ...))
  }
  ...
  if (!is.null(mask)) {
    ...
    results_expanded <- replicate(
      length(mask_lgl),
      getOption("RMINC_MASKED_VALUE"),
      simplify = FALSE
    )
...
vertexLm <- function(formula, data, subset = NULL, column = 1) {
  ...
  if (parseLmOutput$matrixFound) {
    parseLmOutput$data.matrix.left <- vertexTable(
      parseLmOutput$data.matrix.left,
      column = column
    )
    parseLmOutput$data.matrix.right <- vertexTable(
...
vertexLmer <-
  function(
    ...
    if (
      !is.null(mask) &
        (!(is.character(mask) & length(mask) == 1)) &
        !is.numeric(mask)
    ) {
      stop(
        "A numeric vector or file name must be supplied for the mask argument"
      )
    }
    ...
    ## an NA vector of matching shape when a vertex fails. Without this,
    ## a scalar NA mixes with full-length result vectors and corrupts the
    ## collated result matrix.
    sample_vertex_data <- vertexTable(lmod$fr[, 1], column = column)[1, ]
    expected_length <- length(
      mincLmerOptimizeAndExtract(sample_vertex_data, mincLmerList, summary_fun)
    )
...
vertexLmerEstimateDF <-
  function(model, column = 1) {
    ...
    if (is.null(mask)) {
      ...
      !(is.numeric(mask) || is.logical(mask)) || length(mask) != nrow(model)
    ) {
      stop(
        "There is a problem with your mask, please check that it fits your results"
      )
    }
      ...
      dfs[i, ] <-
        suppressMessages(
          tryCatch(summary(mmod)$coefficients[, "df"], error = function(e) {
            warning(
              "Unable to estimate DFs for vertex ",
              rvertices[i],
              call. = FALSE
            )
            NA
  ...
  function(
    ...
    if (is.null(weights)) {
      ...
      if (length(weights) != length(x)) {
        stop("Please supply 1 or enough weights for each element of x")
      }
    }

    if (inherits(surface, "character")) {
      if (length(surface) != 1) {
        stop("Only one file can be passed for the surface")
      }
...
vertexTFCE.vertexLm <-
  function(
    ...
    output <- list(
      tfce = original_tfce,
      randomization_dist = randomization_dist,
      args = list(call = lmod_call, side = side, alternative = alternative)
    )
