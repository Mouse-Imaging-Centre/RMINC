...
read_obj <- function(bic_obj, use_civet_triangles = FALSE) {
  ...
  structure(
    list(vertex_matrix = vertices, triangle_matrix = polygons),
    class = "bic_obj"
  )
}
  ...
  function(
    ...
    mesh <- with(
      ...
      rgl::tmesh3d(
        vertex_matrix,
        triangle_matrix,
        material = list(specular = specular, color = color, ...),
        homogeneous = FALSE
      )
    )

    if (add_normals) {
      mesh <- mesh %>% rgl::addNormals()
    }
...
vals_to_numeric <- function(map) {
  if (is.character(map) && length(map) == 1) {
    ...
    stop(
      substitute(map),
      " must either be a factor vector, numeric vector or a text file path"
    )
  }
  ...
  function(
    ...
    if (!symmetric && !labels) {
      if (is.null(colour_range)) {
        colour_range <- range(colour_map, na.rm = TRUE)
      }
    ...
    if (symmetric && !labels) {
      ...
      palette <- list(
        pos = colorRampPalette(c("red", "yellow"))(colour_depth),
        neg_palette = colorRampPalette(c("blue", "turquoise1"))(colour_depth)
      )

      if (is.null(colour_range)) {
        colour_range <- range(abs(colour_map), na.rm = TRUE)
      }
...
plot.bic_obj <-
  function(
    ...
    if (
      diff(window_dimensions[c(1, 3)]) < 400 &&
    ...
    if (colour_bar && !is.null(colour_map)) {
      do.call("add_colour_bar", c(list(mesh = mesh), par))
    }
...
#' defaults to .95
#' @param nudge_title_y Offset text from the bottom of a colour bar by y, units are
...
add_colour_bar <- function(
  ...
  if (is.null(mesh$legend)) {
    stop("Your mesh has no colour information")
  }
  with(mesh$legend, {
    rgl::bgplot3d({
      par(mar = c(4, 8, 4, 2))
      ...
      if (!symmetric) {
        ...
        text_h <- bpos + (tpos - bpos) * nudge_title_y
        text_w <- rpos + (rpos - lpos) * nudge_title_x
        ...
        text_h <- bpos + (tpos2 - bpos) * nudge_title_y
        text_w <- rpos + (rpos - lpos) * nudge_title_x
...
obj_montage <- function(
  ...
  rgl::par3d(
    viewport = c(
      0,
      0,
      plot_corners[3] - plot_corners[1],
      plot_corners[4] - plot_corners[2]
    )
  )
  ...
  mapply(
    function(subscene, view_matrix, hemisphere) {
  ...
  if (!is.null(output)) {
    rgl::snapshot3d(output)
  }
  if (!is.null(output) && close_on_output) {
    rgl::rgl.close()
  }
...
select_one_vertex <-
  function(object, tolerance, indicate, ...) {
    ...
    if (rect_bounds[1] != rect_bounds[3] || rect_bounds[2] != rect_bounds[4]) {
      warning(
        "Click-and-drag selection does not work with vertexSelect, using position of button press",
        call. = FALSE
      )
    }
    ...
    window_coords <- rgl::rgl.user2window(
      object_vertices,
    ...
    target_location <- rgl::rgl.window2user(
      click_location[1],
      click_location[2],
      0,
      projection = selection$proj
...
vertexLookup <-
  function(
    ...
    if (vertices %>% is("mesh3d")) {
      vertices <- t(vertices$vb)
    }
    if (vertices %>% is("bic_obj")) {
      vertices <- t(vertices$vertex_matrix)
    }
...
#' @export
line_obj_to_voxel <-
  function(line_obj, minc_file) {
    stopifnot(
      inherits(line_obj, "bic_lines"),
      attr(line_obj, "coord_system") == "world"
    )
