  ...
  function(obj_graph, data_map, data_range) {
    subgraph <-
      igraph::induced_subgraph(
        obj_graph,
        igraph::V(obj_graph)[between(data_map, data_range[1], data_range[2])]
      )
...
components_to_map <-
  function(graph, components, type = c("size", "label")) {
    type <- match.arg(type)
    col <- switch(type, size = "clust_size", label = "clust_id")
...
vertexFindPeaks <- function(
  ...
  res <-
    switch(
      ...
      "both" = find_graph_extrema(data_map, `>`, threshold[1]) |
        find_graph_extrema(data_map, `<`, threshold[2])
    )
