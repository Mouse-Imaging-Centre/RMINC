anatGetFile <- function(
  ...
  if (defs == "") {
    stop(
      "No label definitions specified. Either use the defs argument, or use the environment variable $RMINC_LABEL_DEFINITIONS."
    )
  }
  ...
  if (file.access(as.character(defs), 4) == -1) {
    stop(
      "The specified label definitions can not be read: ",
      defs,
  ...
  if (method == "jacobians") {
    ...
    # unrecognized option...
    stop(
      "Unrecognized option used for \"method\" (anatGetFile/anatGetAll). Available options are: jacobians, labels, means, sums, text."
    )
  }
...
anatGetAll <-
  function(
    ...
    if (method == "labels" && !is.null(atlas)) {
      stop(
        "Cannot use labels with an atlas, please pass in label volumes as `filenames`"
      )
    }
    ...
    if (!is.null(defs)) {
      #Read early to allow quick fail out
    ...
    if (is.null(parallel)) {
      ...
      if (length(atlas) > 1) {
        atlas_groups <-
          split(atlas, groupingVector(length(atlas), n_groups))
      } else {
        atlas_groups <- list(atlas)
      }

      file_groups <-
        split(filenames, groupingVector(length(filenames), n_groups))
      ...
      if (parallel[1] %in% c("local", "snowfall")) {
        ...
        reg <- qMincRegistry(
          new_file("anatGet_registry"),
          conf_file = conf_file
        )
        on.exit(tenacious_remove_registry(reg))

        suppressWarnings(
          #Warning suppression for large env for function (>10mb)
          ids <- batchMap(
            reg = reg,
            function(group) {
              compute_summary(group)
    ...
    if (is.null(defs)) {
      warning(
        "No definitions provided, using indices from atlas \n",
  ...
  function(defs, side = c("both", "left", "right"), hierarchy = NULL) {
    ...
    if (!is.null(hierarchy) && hierarchy %in% names(label_defs)) {
      label_defs <-
        select(
          label_defs,
          !!!c("Structure", "left.label", "right.label", hierarchy)
        ) %>% ##new tidyeval
        rename(hierarchy = !!hierarchy)
    } else {
      label_defs <- select(
        label_defs,
        !!!c("Structure", "left.label", "right.label")
      )
    }
    ...
    if ("hierarchy" %in% names(label_defs)) {
      label_defs <-
        ...
        mutate(
          hierarchy = case_when(
            is.na(.data$hierarchy) | .data$hierarchy == "" ~ "",
            both_sides ~ hierarchy,
            .data$hemisphere == "right.label" ~
              paste0("right ", .data$hierarchy),
    ...
    label_defs <-
      switch(
        ...
        left = filter(
          label_defs,
          .data$both_sides | .data$hemisphere == "left.label"
        ),
        right = filter(
          label_defs,
          .data$both_sides | .data$hemisphere == "right.label"
        ),
        both = label_defs
      )
...
create_anat_results <-
  function(
    ...
    if (nrow(results) == 1) {
      stop("No structures found, check your atlas or label volumes")
    }
    ...
    if (!is.null(definitions)) {
      attr(results, "definitions") <- definitions
    }
...
print.anatModel <- function(
  ...
  sub_x <- x[seq_len(n), seq_len(width)]
  attributes(sub_x) <- attributes(sub_x) %>%
    {
      .[names(.) %in% c("dim", "dimnames")]
    }
...
`[.anatModel` <- function(x, i, j, drop = TRUE) {
  ...
  mdrop <- missing(drop)
  n_args <- nargs() - !mdrop
  model_attrs <- attributes(x) %>%
    {
      .[
        !names(.) %in%
          c(attributes_to_skip, "dimnames", "class", "df", "stat-type")
      ]
    }
  class(x) <- "matrix"
  ...
  if (!is.null(dim(x))) {
    ...
    if (length(orig_stats) == 1) {
      orig_stats <- rep(orig_stats, ncol(orig_x))
    }
    stat_columns <- which(orig_stats %in% knownStats)
    selected_stat_columns <- match(j, stat_columns, 0)

    if (length(orig_dfs) == 1) {
      orig_dfs <- rep(orig_dfs, length(stat_columns))
    }
...
rbind.anatModel <- function(..., deparse.level = 1) {
  args <- list(...)
  attrs <- lapply(args, function(model) {
    attributes(model) %>%
      {
        .[!names(.) %in% "call"]
      }
  ...
  lapply(attrs, function(att) {
    if (!identical(att, attrs[[1]])) {
      stop("Models aren't compatible, differ in some attributes")
    }
  })
...
anatGetAll_old <- function(
  ...
  for (i in 1:length(filenames)) {
    ...
    if (length(diff_between_file_and_mapping) > 0) {
      print(paste(
        "WARNING: Labels found in the inputfile, but not in the mapping: ",
        diff_between_file_and_mapping
      ))
    }
    ...
    if (length(diff_between_mapping_and_file) > 0) {
      print(paste(
        "WARNING: Labels found in the mapping, but not in the file: ",
        diff_between_mapping_and_file
      ))
    }
...
#' }
#' @export
anatCombineStructures <- function(
  vols,
  method = "jacobians",
  defs = getOption("RMINC_LABEL_DEFINITIONS")
) {
  ...
  for (i in 1:nrow(labels)) {
    if (labels$right.label[i] == labels$left.label[i]) {
      combined.labels[, i] <- vols[,
        labelNumbers == as.character(labels$right.label[i])
      ]
    } else {
...
anatLm <- function(formula, data, anat, subset = NULL, weights = NULL) {
  ...
  if (matrixFound) {
    stop(
      "A term in your model is a matrix, this use of `anatLm` is deprecated, ",
  ...
  if (!is.null(weights)) {
    if (length(weights) != nrow(mmatrix)) {
      stop("Incorrect number of weights supplied, need one per observation")
    }
    result <- .Call(
      "vertex_wlm_loop",
      data.matrix.left,
...
anatLmer <-
  function(
    ...
    rho$pp <- do.call(
      merPredD$new,
      c(
        lmod$reTrms[c("Zt", "theta", "Lambdat", "Lind")],
        n = nrow(lmod$X),
        list(X = lmod$X)
      )
    )
    ...
    attr(out, "logLikDF") <- length(mmod@beta) +
      length(mmod@theta) +
      mmod@devcomp[["dims"]][["useSc"]]
    attr(out, "REML") <- REML
...
anatLmerEstimateDF <- function(model, n = 50, verbose = FALSE) {
  ...
  dfs <- matrix(
    nrow = nstructures,
    ncol = sum(attr(model, "stat-type") %in% "tlmer")
  )
  ...
  for (i in seq_len(nstructures)) {
    ...
    dfs[i, ] <-
      suppressMessages(
        tryCatch(summary(mmod)$coefficients[, "df"], error = function(e) {
          warning(
            "Unable to estimate DFs for structure ",
...
#' }
anatToVolume <- function(
  anat,
  labelVolume,
  column,
  defs = attr(anat, "definitions")
) {
  labels <- c()
  values <- c()
