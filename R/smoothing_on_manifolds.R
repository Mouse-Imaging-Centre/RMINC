...
find_neigh_verts = function(idx, manifold, clean = F) {
  ...
  ret = as.vector(triangle_matrix[, apply(triangle_matrix, 2, function(i) {
    any(i == idx)
  })])
  ret = unique(ret)
  if (clean) {
...
find_n_neigh_verts = function(idx, n, manifold, clean = F) {
  ...
  while (TRUE) {
    ...
    ret = unique(unlist(lapply(ret, function(i) {
      find_neigh_verts(i, manifold, clean = F)
    })))
  }
  if (clean) {
    ret = sort(ret)
...
filter_problematic_triangles = function(edge, manifold, k = 1) {
  ...
  num_neigh = unlist(lapply(test_verts, function(i) {
    find_num_n_neigh_vert(i, k, manifold)
  }))
...
clean_up_manifold_triangles = function(manifold, k = 1) {
  ...
  if (length(problematic_edges) != 0) {
    ...
    problematic_triangle_idxs = unlist(lapply(
      ...
      function(i) {
        filter_problematic_triangles(problematic_edges[i, ], manifold, k = k)
      }
    ))

    manifold$triangle_matrix = manifold$triangle_matrix[,
      -problematic_triangle_idxs
    ]
  }
  return(manifold)
...
find_edgelengths_sq = function(manifold) {
  ...
  triangle_matrix = manifold$triangle_matrix
  apply(triangle_matrix, 2, function(i) {
    triangle_to_edgelengths_sq(vertex_matrix[, i])
  })
}
...
find_laplace_beltrami_nonzero_elements = function(manifold) {
  ...
  tst = edges %>%
    ...
    nrow
  if (tst != 0) {
    stop('some problematic edges remain')
  }
  ...
  ret_diag_edges = cbind(tmpres[, 1], tmpres[, 1])
  colnames(ret_diag_edges) = c('i', 'j')
...
laplace_beltrami_operator = function(
  ...
  if (
    is.null(dim(manifold$vertex_matrix)) |
      is.null(dim(manifold$triangle_matrix))
  ) {
    defense_func(3)
    cat('\n')
    stop('Error in input arguments')
  ...
  if (
    (nrow(manifold$vertex_matrix) != 3) | (nrow(manifold$triangle_matrix) != 3)
  ) {
    defense_func(3)
    cat('\n')
    stop('Error in input arguments')
  ...
  manifold$laplace_beltrami_elements = find_laplace_beltrami_nonzero_elements(
    manifold
  )
  ...
  laplace_beltrami_operator =
    sparseMatrix(
      ...
      x = manifold$laplace_beltrami_elements$elements,
      dims = list(n, n)
    )
...
#'}
#' @export
laplace_beltrami_smoothing = function(
  manifold,
  scalar_field,
  fwhm,
  maxiter = 1000
  ...
  for (i in 1:maxiter) {
    if ((i %% 10) == 0) {
      cat(
        ...
        i,
        'Current FWHM:',
        tsim_to_fwhm(t_current),
        'Target FWHM:',
        fwhm,
        '\n'
      )
    }
    tstep_log10 = tstep_size_select(
      lbmat,
      scalar_field,
      min_scal_val,
      max_scal_val
