...
mincLmer <- function(
  ...
  if (!is.character(mask)) {
    stop("A character mask must be provided to run mincLmer")
  }
  ...
  if (!is.null(parallel)) {
    ...
    out <- mincApplyRCPP(
      lmod$fr[, 1], # assumes that the formula was e.g. filenames ~ effects
      optimizer_fun,
      mincLmerList = mincLmerList,
      mask = mask,
      summary_fun = summary_fun,
    ...
    dfs[i, ] <-
      suppressMessages(
        tryCatch(summary(mmod)$coefficients[, "df"], error = function(e) {
          warning(
            "Unable to estimate DFs for voxel (",
...
fixef_summary <- function(mmod) {
  se <- tryCatch(
    {
      # vcov sometimes complains that matrix is not positive definite
...
ranef_summary <-
  function(mmod) {
    ...
    betas_and_ts <-
      mapply(
        function(e, group_name) {
          condVar <- attr(e, "postVar")
          group_se <- apply(condVar, 3, function(v) sqrt(diag(as.matrix(v))))

          if (is.null(dim(group_se))) {
            #deal with the dropped dimension in case of two group ranef
...
mincLogLikRatio <- function(...) {
  ...
  for (i in 1:length(dots)) {
    ...
    if (is.null(REML)) {
      ...
      stop(
        "Log likelihood ratio tests only work reliably if fitted with maximum likelihood, but not if fitted with restricted maximum likelihood. Rerun your model with REML=FALSE"
      )
    }
  }
...
#' @export
mincLogLikRatioParametricBootstrap <- function(
  logLikOutput,
  selection = "random",
  nsims = 500,
  nvoxels = 50
  ...
  if (length(mincLmerLists) != 2) {
    stop(
      "Error: parametric bootstrap only implemented for the two model comparison case"
    )
  }
  if (selection == "random") {
    ...
    voxelMatrix <- matrix(
      nrow = nsims,
      ncol = nrow(attr(logLikOutput, "mincLmerLists")[[1]][[1]]$X)
    )
    ...
    for (i in 1:nvoxels) {
      ...
      voxel <- mincGetVoxel(
        mincLmerList[[1]]$fr[, 1],
        mincVectorToVoxelCoordinates(mask, voxels[i])
      )
      ...
      for (j in 1:nsims) {
        ...
        simLogLik[j, 1] <- logLik(mincLmerOptimize(
          voxelMatrix[j, ],
          mincLmerList
        ))
      }
      ...
      for (j in 1:nsims) {
        simLogLik[j, 2] <- logLik(mincLmerOptimize(
          voxelMatrix[j, ],
          mincLmerList
        ))
      }
      ...
      out[i, 1] <- pchisq(
        logLikOutput[voxels[i]],
        attr(logLikOutput, "df"),
        lower.tail = FALSE
      )
