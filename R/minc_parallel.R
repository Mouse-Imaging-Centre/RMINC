create_parallel_mask <-
  function(
    ...
    if (is.null(mask)) {
      if (n %% 1 != 0) {
        stop("the number of jobs must be an integer")
      }
...
tenacious_remove_registry <-
  function(reg) {
    tryCatch(removeRegistry(reg = reg), error = function(e) {
...
pMincApply <-
  function(
    ...
    if (!is.null(method)) {
      warning(
        "Use of method is deprecated, please use a `conf_file` or",
        "the local argument"
      )
    }
...
mcMincApply <-
  function(
    ...
    if (is.null(mask_vals)) {
      mask_file <- create_parallel_mask(
    ...
    if (return_raw) {
      ...
      results <- setMincAttributes(
        results,
        list(filenames = filenames, likeVolume = filenames[1], mask = mask)
      )
      return(results)
    }
    ...
    if (!is.null(mask)) {
      expanded_results <- rep(
        list(getOption("RMINC_MASKED_VALUE")),
        length(sample_volume)
      )
...
qMincApply <-
  function(
    ...
    qMinc_registry <-
      qMincRegistry(
        registry_name = new_file(registry_name, registry_dir),
        registry_dir = registry_dir,
        packages = packages,
        clobber = clobber,
        resources = resources,
...
qMincReduce <-
  function(
    ...
    if ((!ignore_incompletes) && nrow(findNotDone(reg = registry)) != 0) {
      stop(
        "Some jobs have not terminated, use `ignore_incompletes` to reduce anyway, or set `wait`"
      )
    }
    ...
    results <- unlist(lapply(results, function(el) el$vals), recursive = FALSE)[
      result_order
    ]
