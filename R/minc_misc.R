...
minc.isReadable <- function(filenames) {
  ...
  READ_PERMISSION <- 4
  if (
    sum(file.access(as.character(filenames), READ_PERMISSION)) != 0 ||
      is.null(filenames)
  ) {
...
asMinc2 <- function(filename, output, clobber = FALSE) {
  ...
  if (!isMinc(filename)) {
    stop(paste(
      "Error: Trying to convert a non-minc file (",
      filename,
      ") to minc",
      sep = ""
    ))
  ...
  cmdOptions <- "-2"
  if (clobber) {
    cmdOptions <- paste(cmdOptions, "-clobber")
  }
...
mincGetDataTypes <- function() {
  ...
  dataTypes.df <- data.frame(
    code = dataType_code,
    numCode = dataType_numCode,
...
mincGetDataClasses <- function() {
  ...
  dataClasses.df <- data.frame(
    numCode = dataClass_numCode,
    string = dataClass_string,
    stringsAsFactors = FALSE
  )
...
rminc.readLinearXfmFile <- function(xfmFilename) {
  ...
  if (!status) {
    stop("Program xfm2param of package mni_autoreg cannot be found on PATH")
  }
...
rminc.checkForExternalProgram <- function(
  program,
  test_string,
  prog_options = ""
) {
  ...
  if (!grepl(test_string, cmdOutLong, fixed = TRUE)) {
    ...
    cat(sprintf(
      "Attempt to execute program \"%s\" within shell failed\n",
      program
    ))
...
volume.explodeLabelVolume <- function(
  label_vol,
  labels = NULL,
  civetLabels = TRUE
) {
  ...
  if (is.null(labels)) {
    ...
    if (!is.vector(labels, mode = "numeric")) {
      stop("Label vector must be a numeric vector")
    }
  }
  ...
  for (label in labels) {
    ...
    if (civetLabels) {
      label_name <- tissueTypes[label + 1]
    }
...
fast.qvalue <- function(
  ...
  if (min(p) < 0 || max(p) > 1) {
    if (gui) {
      eval(
        expression(postMsg(paste("ERROR: p-values not in valid range.", "\n"))),
        parent.frame()
      )
    } else {
  ...
  if (length(lambda) > 1 && length(lambda) < 4) {
    if (gui) {
      eval(
        expression(postMsg(paste(
          "ERROR: If length of lambda greater than 1, you need at least 4 values.",
          "\n"
        ))),
        parent.frame()
      )
  ...
  if (length(lambda) > 1 && (min(lambda) < 0 || max(lambda) >= 1)) {
    if (gui) {
      eval(
        expression(postMsg(paste(
          "ERROR: Lambda must be within [0, 1).",
          "\n"
        ))),
        parent.frame()
      )
  ...
  if (length(lambda) == 1) {
    ...
    if (pi0.method == "smoother") {
      ...
      for (i in 1:100) {
        ...
        for (i in 1:length(lambda)) {
          pi0.boot[i] <- mean(p.boot > lambda[i]) /
            (1 -
              lambda[i])
        }
  ...
  if (!is.null(fdr.level) && (fdr.level <= 0 || fdr.level > 1)) {
    if (gui) {
      eval(
        expression(postMsg(paste(
          "ERROR: 'fdr.level' must be within (0, 1].",
          "\n"
        ))),
        parent.frame()
      )
