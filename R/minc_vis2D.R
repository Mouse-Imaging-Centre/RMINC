...
mincArray <- function(volume, dimIndex = 1, maskVolume = NULL) {
  ...
  if (!is.null(maskVolume)) {
    ...
    stop(
      "Input needs to either have dimensions set, or have a sizes or likeVolume attribute"
    )
  }
  return(outvol)
}
...
plotLocator <- function(dimension, anatomy, indicatorLevels, slices) {
  ...
  if (length(d) != 3) {
    stop(
      "anatomy must be 3 dimensional, you may be missing a call to mincArray"
    )
  }
...
sliceSeriesLayout <-
  function(anatomy, dimension, mfrow, begin, end, plottitle, legend, locator) {
    ...
    if (locator || !is.null(legend)) {
      ...
      layoutMatrix <- cbind(
        matrix(1:nslices, nrow = mfrow[1], ncol = mfrow[2], byrow = T),
        c(nslices + 1, rep(nslices + 2, mfrow[1] - 1))
      )
      layout(layoutMatrix)
    } else {
      # go with standard mfrow setting
      par(mfrow = mfrow)
    }
    ...
    d <- dim(anatomy)
    if (length(d) != 3) {
      stop("anatomy must be 3 dimensional")
    }
...
mincPlotSliceSeries <-
  function(
    ...
    mfrow = c(4, 5), # layout
    low = NULL,
    high = NULL, # stat thresholding
    anatLow = NULL,
    anatHigh = NULL, # anatomy thresholding
    col = heat.colors(255),
    ...
    argument_list <- #Eval them in this environment
      lapply(dispatch_function_formals, function(formal) {
        eval(as.symbol(formal))
      })
...
mincPlotSimpleSliceSeries <-
  function(
    ...
    if (locator) {
      plotLocator(dimension, anatomy, indicatorLevels, slices)
    }
...
mincPlotStatsSliceSeries <-
  function(
    ...
    if (discreteStats) {
      statRange <- c(
        max(min(statistics, na.rm = TRUE), low),
...
getRangeFromHistogram <- function(volume, low = NULL, high = NULL) {
  if (is.null(low) || is.null(high)) {
    hist_midpoints <- hist(volume, plot = F)$mids
  }
  if (is.null(low)) {
    low <- hist_midpoints[5]
  }
  if (is.null(high)) {
    high <- rev(hist_midpoints)[5]
  }
...
#' @return invisible NULL
#' @export
mincTriplanarSlicePlot <- function(
  anatomy,
  statistics,
  slice = NULL,
  layoutMatrix = NULL,
...
#' @return invisible NULL
#' @export
mincPlotAnatAndStatsSlice <- function(
  anatomy,
  statistics,
  slice = NULL,
  dimension = 2,
...
mincImage <- function(
  ...
  if (!add) {
    plot.special <- #override plot.defaults defaults
      function(..., xlab = "", ylab = "", asp = 1, xaxs = "i", yaxs = "i") {
        plot.default(
          ...,
          xlab = xlab,
          ylab = ylab,
...
mincRayTraceStats <- function(
  ...
  lasterr <- try(
    system("ray_trace_crosshair", ignore.stderr = TRUE),
    silent = TRUE
  )
  if (lasterr == 32512) {
  ...
  if (class(anatomy.volume)[1] == "character") {
    ...
    stop(
      "Please specify the path to the anatomy volume, or a mincSingleDim containing the anatomy volume."
    )
  }
...
lut_to_palette <- function(
  ...
  lut <- lut[, 2:4]
  crp <- colorRampPalette(apply(lut, 1, function(x) {
