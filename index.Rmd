...
RMINC is a package for the R statistical environment designed to read and write MINC volumes as well as perform optimized statistical routines at every voxel (or vertex) of a set of files. It supports reading and writing volumes; voxel-wise and surface/vertex-wise linear and linear mixed-effects models; correlations, t-tests, ANOVA, and Wilcoxon tests; False Discovery Rate correction and Threshold-Free Cluster Enhancement (TFCE); ROI-based and hierarchical-atlas anatomy analysis; and parallel execution on local cores or HPC clusters via `batchtools`.
...
see the [INSTALL](https://github.com/Mouse-Imaging-Centre/RMINC/blob/develop/INSTALL) file
...
[Analyzing Anatomy with Hierarchical Atlases](https://raw.githubusercontent.com/Mouse-Imaging-Centre/RMINC/develop/inst/documentation/hierarchiesTutorial.html) instead of the ROI based analysis, and [How RMINC Parallel Works](https://raw.githubusercontent.com/Mouse-Imaging-Centre/RMINC/develop/inst/documentation/RMINC_Parallelism.html)
...
These instructions are largely superseded by the [parallel tutorial](https://raw.githubusercontent.com/Mouse-Imaging-Centre/RMINC/develop/inst/documentation/RMINC_Parallelism.html).

pMincApply, just as mincApply, can be used to run any function on all voxels in your input files. The difference with mincApply, is that pMincApply can be parallelized (hence the p). You can use batchtools to run it locally using multiple cores on your machine, or submit jobs to a batch system. Here is a short example of how to use pMincApply to run a function on your input files that is not implemented by any of the standard MINC functions.
