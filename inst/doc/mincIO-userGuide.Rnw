...
%%\VignetteIndexEntry{An introduction to the mniMincIO package}
...
\title{\textbf{\Huge{RMINC Module \textsf{mincIO}}} \\[1 cm] 
...
\section*{The \textsf{RMINC-mincIO} Module}
As you may have guessed from the title of this document, it was indeed possible to expand on the work done by Jason Lerch with the \textsf{RMINC} package, and that thing is called \textsf{mincIO}.  \textsf{mincIO} differs from and extends the original \textsf{RMINC} in a number of ways.  Firstly, as suggested by the module name, \textsf{mincIO} deals exclusively with MINC IO functions.  No other non-IO-related stuff is included, and it never will be.  Other packages that require \textsf{mincIO} functionality can simply list \textsf{RMINC} as a package dependancy, causing the \textsf{mincIO} functions to load immediately prior to the dependent package loading.  

\textsf{mincIO} also differs from \textsf{RMINC} in that \textsf{mincIO} makes use of R's S4 classes instead of the S3 classes used by base \textsf{RMINC}.  Now this is a rather technical issue, so let it suffice to say that S4 classes permit us to define fairly classical (more or less) classes, such as one might use in C++.  All of the various \textsf{mincIO} S4 classes can be easily instantiated and used. It's probably a good thing to get a fairly clear sense of what these objects are, and how they interact.  A quick peruse of the \textsf{RMINC-mincIO} reference guide or online help should provide a quick overview.

This reminds me that every function, method, and class that comprises the \textsf{mincIO} package has been documented and is available via the online R help.  The R help system can usually be easily accessed via menus (if you're using a version of R with a graphical user interface), else typing \texttt{help.start()} at the R command-line should open a browser page.  From the browser page clicking on \texttt{packages} and then \textsf{RMINC}, should take you to the online help.  In addition, all of the \textsf{mincIO} help pages should also be available in pdf form, so get a hold of them and enjoy countless hours of bathroom reading.


\section*{Using RMINC-mincIO}
This section of the document serves as a tutorial, packed with examples of \textsf{mincIO} usage.  Before we get on with the tutorial goodness, first a bit of implementation background.  

\paragraph{An Overview.} As was the case with \textsf{RMINC}, \textsf{mincIO} is built using the MINC2 API. What does this mean?  Basically, it means that whenever \textsf{mincIO} accesses a MINC2 volume, \textsf{mincIO} repackages the passed call arguments and then passes off control to a shared library (coded in C), which does a bit more processing, calling the MINC2 API, as needed, and then returns control.  The calls to the MINC2 library routines require that your system have the BIC MINC tools installed, specifically, the MINC2 library needs to be build as a dynamic library, in order to permit real-time loading of the MINC2 API routines into R's address space.  So, make sure that MINC is installed on your system, and that MINC2 is built as a dynamic library.  On a related note, the MINC command-line tools should also be installed and available on your \texttt{PATH}, since \textsf{mincIO} occasionally forks off some calls to these tools (e.g., \texttt{mincconvert} for conversion to MINC2, \texttt{mincinfo} to grab some volume attributes).

\paragraph{\textsf{mincIO} does \emph{not} do Windows\copyright.} Now, the previous paragraph should make clear that, given that MINC needs to be installed, and given that the MINC tools are not really supported on the Windows platform, running \textsf{mincIO} under Windows is not really an option.  This is not an R restriction, as R is multi-platform, but rather a MINC restriction. As such, \textsf{mincIO} has not been tested under Windows, has never compiled under Windows, and is therefore not supported under Windows.  Of course, I would very much like \textsf{mincIO} to run under Windows, and I'm entirely open to adding code to the package that would make this possible. Volunteers?

\paragraph{The MINC2 API.} This API was created in order to present the user\footnote{The ``user'' being a C programmer.} with a simpler and more convenient API to MINC volume access than was previously available via the original MINC API.  In addition, unlike the original MINC API that was built on top of the NetCDF API, MINC2 was built using the HDF5 API.  The consequence of this decision is that the MINC2 API is not capable of reading the older NetCDF-based (MINC1) volumes.  Happily, the \texttt{mincconvert} MINC utility does a fabulous job of losslessly converting MINC1 to MINC2 volumes, and the \textsf{mincIO} package does this conversion auto-magically whenever it detects a MINC1 volume.  Of course, this conversion does take some time, and so \textsf{mincIO} spits out a warning message to let you know that it's doing a conversion.  Clearly, if you know that you're using MINC1 volumes, it would probably be a good idea to run \texttt{mincconvert} yourself prior to running your R code.\footnote{You can tell whether you're dealing with MINC1 or MINC2 by using the \texttt{file} command-line tool. For example, type ``\texttt{file myMincVolume.mnc}'', and if the returned line contains ``NetCDF'', then it's MINC1; if it contains ``HDF5'', then it's MINC2.  Note that the Civet pipeline currently produces MINC2 volumes (a very good thing).}
...
Loading \textsf{RMINC} causes a couple of other required packages to be loaded (e.g., lattice, grid).  If you don't have these, go and install them now --- I'll wait for you.   Done yet?  Once you've loaded the package, try displaying the package overview document.
...
<<echo=TRUE, keep.source=TRUE>>=
# see doc with: "?readVolume"
#
# load the ICBM-152 volume
vIcbm <- "icbm_avg_152_t1_tal_lin.mnc"
vIcbm <- file.path(volDir, vIcbm)
vol <- readVolume(vIcbm)
print(vol)
@
So we simply create a string containing the full path and file name, and call the \texttt{readVolume()} function, which returns a MincVolumeIO object in variable \texttt{vol}.  As before, we use the standard (overloaded) \texttt{print()} function to print the variable information. To get a visual summary of the volume, just call the \texttt{plot()} function, passing the new \texttt{vol} variable, the results of which can be seen in Figure~\ref{fig:volumePlot}.

% kludge to generate png graphics rather than pdf
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/volumePlot.png')
@
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/volumePlot.png}
...
vol2 <-asVolume(maskedImage, vol)

# combining previous 2 lines into 1
vol2 <- asVolume(vol * mask3D, vol)
@

% kludge to generate png graphics rather than pdf
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/volumePlotMasked.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE>>=
plot(vol2)
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/volumePlotMasked.png}
...
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/volumePlotLine.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE>>=
plot(vol)
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/volumePlotLine.png}
...
rangeX <- numeric()
rangeX[1] <- convertWorldToVoxel(vIcbm, c(0-30, 0, 0))[1]
rangeX[2] <- convertWorldToVoxel(vIcbm, c(0+30, 0, 0))[1]
#
# ... Y-axis start and stop voxel (-60 mm, +30 mm from AC)
rangeY <- numeric()
rangeY[1] <- convertWorldToVoxel(vIcbm, c(0, 0-60, 0))[2]
rangeY[2] <- convertWorldToVoxel(vIcbm, c(0, 0+30, 0))[2]
#
# ... Z-axis start and stop voxel (-20 mm, +30 mm from AC)
rangeZ <- numeric()
rangeZ[1] <- convertWorldToVoxel(vIcbm, c(0, 0, 0-20))[3]
rangeZ[2] <- convertWorldToVoxel(vIcbm, c(0, 0, 0+30))[3]
#
# ... create a new initialized mask volume and create hyperslab mask
maskVol <- makeNewVolume(filename="mask_volume.mnc", likeTemplate="icbm152")
maskVol[ rangeX[1] : rangeX[2],
         rangeY[1] : rangeY[2],
         rangeZ[1] : rangeZ[2] ] <- 1
#
# ... apply the mask to the T1 volume and recast as volume
hyperVol <- asVolume(vol * maskVol, vol)
@

Note the new \texttt{convertWorldToVoxel()} function, that takes a volume name and a vector of world coordinates (XYZ order).  Perhaps in the near future, we'll be able to pass it a MincVolumeIO object, saving us some file IO, but sadly that day has not yet come.  Also new is the \texttt{makeNewVolume()} function, which creates a new (initialized to zeros) volume, with the attributes set according to a known template (ICBM, in this case).  Figure~\ref{fig:volumePlotHslab} displays the ROI within the masked volume.

% kludge to generate png graphics rather than pdf
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/volumePlotHslab.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE>>=
plot(hyperVol)
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/volumePlotHslab.png}
...
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/axialSlice01.png')
@
% ... send plot out to png device
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/axialSlice01.png}
...
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/axialSlice02.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE, keep.source=TRUE>>=
plot(s055)                         # visualize slice 55
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/axialSlice02.png}
...
The results of the modifications are can be seen in Figure~\ref{fig:axialSlice02}.  The final step would be to move the modified slice back into the MincVolumeIO object prior to writing the modified 3D object back to disk.  This is easily done via the \texttt{putSlice()} function, and looks like the following.

<<echo=TRUE, keep.source=TRUE>>=
vol <- putSlice(s055, vol)        # update vol with moded slice
@
Note that the only mandatory inputs to \texttt{putSlice()} are the slice and the volume to be updated.  The slice number need not be specified, as the slice already ``knows'' its slice number, and therefore puts itself back into the correct place within the volume.  An explicit specification of the slice number (3rd argument) allows the user to write the slice back into a different position within the volume (although the dimensionality must still match).  In order to prove that the updated slice \emph{has} been written to the volume, let's take a look a the volume plot shown in Figure~\ref{fig:volumePlotUpdatedSlice}.

% kludge to generate png graphics rather than pdf
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/volumePlotUpdatedSlice.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE, keep.source=TRUE>>=
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/volumePlotUpdatedSlice.png}
...
@
<<echo=TRUE, keep.source=TRUE>>=
# see doc with: "?readBySlice"
#
volumes <- c(vol1, vol2, vol3, vol4, vol5)      # set volume names
slice_array_s46 <- readBySlice(volumes, 46)     # load slice 46
print(slice_array_s46)
@

In addition to the common volume information (common to all volumes) the \texttt{print()} function also shows some slice array details, including, slice number, matrix dimensions, and volume names.\footnote{The astute reader will notice that all of the volumes are the same.  Yes, I cheated by reading the same volume 5 times, since distributing \textsf{mincIO} with a bunch of sample volumes was really not a good thing. Corners needed to be cut.}  As the next step, let's say that you needed to extract a particular slice from the slice array for visualization, manipulation, etc. How would one do that?  Let's try the aptly named \texttt{getSliceFromSliceArray()} function.

<<echo=TRUE, keep.source=TRUE>>=
# see doc with: "?getSliceFromSliceArray"
#
vol3s46 <- getSliceFromSliceArray(slice_array_s46, 3)
print(vol3s46)
@
So there you have it.  Slice 46 from the 3rd volume is now instantiated as a slice object, which we are free to either modify, move to a MincVolumeIO object, or simply visualize --- as we see in Figure~\ref{fig:sliceIO01}.


% kludge to generate png graphics rather than pdf
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/sliceIO01.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE, keep.source=TRUE>>=
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/sliceIO01.png}
\caption[]{\label{fig:sliceIO01} Slice 46 from the 3rd volume, as extracted from the slice array.}
\end{center}
\end{figure}
...
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/sliceIO02.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE, keep.source=TRUE>>=
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/sliceIO02.png}
...
voxCoords <- round(convertWorldToVoxel(vIcbm, worldCoords))
print(voxCoords)
#
voxel_array <- readByVoxel(volumes, voxCoords)
print(voxel_array)
print(voxel_array[,])
@

Ok, it's pretty straightforward.  We're endeavouring to read the real value stored at the voxel located at the mid anterior commissure across 5 volumes.  Actually, all 5 volumes are the same (didactic license again), so the values all better be the same.  As all access uses voxel coordinates, we first convert the stereotactic address to voxel space, and then print it out.\footnote{Be aware that although the \textsf{mincIO} package is perfectly capable of reading and processing native-space volumes, note that the space conversion routines may not be of much use to you unless you're in stereotactic space.  Specifically, in native space, one can get all sorts of unusual Start values, since the origin is machine relative; this can result in funny (e.g., negative) voxel indices being generated by \texttt{convertWorldToVoxel()}, which are usually not super useable.} The actual read is accomplished with \texttt{readByVoxel()}, followed by a print of the voxel array meta information, and then the actual array values themselves. Easy as $\pi$.  We can do the exact same thing with 4D volumes.  Take a boo.

<<echo=TRUE, keep.source=TRUE>>=
volumes <- c(v4D, v4D, v4D, v4D, v4D)

worldCoords <- c(0,0,0)           # get AC voxel coords
voxCoords <- round(convertWorldToVoxel(v4D, worldCoords))
print(voxCoords)
...
% ... open png output
<<fig=FALSE,echo=FALSE>>=
png(filename='figures/voxelIOtac.png')
@
% ... send plot out to png device
<<fig=FALSE,echo=TRUE, keep.source=TRUE>>=
xyplot(voxel_array[1,] ~ round(round(voxel_array@mincInfo@timeOffsets)/60),
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=12cm,draft=false,clip=true]{figures/voxelIOtac.png}
...
#
	...
	mincInfo <- readMincInfo(args[1]) # read the volume info
