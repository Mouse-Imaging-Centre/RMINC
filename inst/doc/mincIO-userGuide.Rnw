...
Now for the easier and recommended way of doing the above.  Specifically, the better way involves use of the \texttt{mincIO.getProperty()} set of methods, which have conveniently been defined for all mincIO objects.  So let's do exactly what we just did, only using the new accessor methods.
