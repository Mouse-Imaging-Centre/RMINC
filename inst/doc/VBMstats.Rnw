...
# retain the current directory. If setwd() is used in a Sweave 
# document, and not reset to the original directory, Sweave will
# complain at the end about not being able to find the .tex file
# that is created
originalDir <- getwd()
...
mincWriteVolume(qvals, fileOut, "tvalue-GenderMale", like.filename="volumes/mask.mnc")
@ 
...
#
attr(volume, "stat-type") <- c("t")
attr(volume, "df") <- c(21)
qvals <- mincFDR(volume, df=21, statType="t", method="FDR")
print(qvals)
@
