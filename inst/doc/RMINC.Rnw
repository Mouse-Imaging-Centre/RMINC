...
\title{Voxel-wise morphometry using RMINC}
\author{Jason Lerch}
\maketitle
...
Morphometry (VBM) or any method in which the goal is to produce a
statistical test at every voxel of a series of files. The assumption
is that the data of interest at every voxel is a scalar (i.e. a single
value rather than a vector).
...
<<hist,fig=TRUE>>=
first.file <- minc.get.volume(gf$file[1])
hist(first.file)
@

Two things happened here. The first was a call to
...
The second command plots a histogram of the entire volume. Fairly
self-explanatory.
...
<<mean>>=
mean.map <- minc.apply(gf$file, quote(mean(x)))
minc.write.volume("/tmp/mean.mnc", gf$file[1], mean.map)
#hist(mean.map)
@ 

This is what happened: the R function \texttt{mean} was applied at
every voxel to of all files, producing a 1D array called
\texttt{mean.map}. The command \texttt{minc.apply} needs two
...
After the mean was computed, it was written to a MINC2
file. \texttt{minc.write.volume} needs three argumets: a string
containing the filename to be written to (note that it will produce an
error if this file exists), a string containing an existing MINC2 file
which the new output will be like (think dimensions, step sizes,
etc.), and the variable containing the data.
...
t <- minc.apply(gf$file, quote(f(x)), mask)
minc.write.volume("/tmp/t-test.mnc", gf$file[1], t)
\end{Sinput}
...
\texttt{minc.apply} is tremendously powerful since it allows any R
...
example, the above bit of code takes X seconds to complete. The
