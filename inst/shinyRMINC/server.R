...
shinyServer(function(input, output, clientData, session) {
  ...
  observe({
    ...
    low_thresh <-
      as.numeric(
        quantile(abs(currentStat[currentStat != 0]), probs = .5)
      )
  ...
  observeEvent(input$fdr_thresh, {
    if (!is.null(fdr) && input$fdr_thresh != "none") {
      ...
      updateSliderInput(
        session,
        "range",
        value = c(tholds[thresh, input$statistic], input$range[2])
      )
    }
  })
  ...
  output$seriesPlot <- renderPlot(
    {
      mincPlotSliceSeries(
        ...
        low = input$range[1],
        high = input$range[2],
        begin = input$sliceRange[1],
        end = input$sliceRange[2],
        plottitle = input$statistic,
        dim = as.integer(input$dimension),
        symmetric = statsList[[input$statistic]]$symmetric,
        legend = statsList[[input$statistic]]$legendTitle,
        mfrow = c(input$rows, input$columns)
      )
    },
    bg = "black"
  )
  ...
  output$graphPlot <- renderPlot({
    ...
    update_geom_defaults(
      "boxplot",
      list(colour = "white", fill = "transparent")
    )
  ...
  output$volumesTable <- DT::renderDataTable({
    ...
    dt <- as.data.frame(t(apply(gfs$vols, 2, function(x) {
      tapply(x, gfs[, "newGrouping"], mean)
    })))
    dtsd <- as.data.frame(t(apply(gfs$vols, 2, function(x) {
      tapply(x, gfs[, "newGrouping"], sd)
    })))
    l <- levels(gfs[, "newGrouping"])
    for (i in 2:length(l)) {
      cat("pre dt\n")
      dt[, paste("effect size:", l[i])] <- (dt[, l[i]] - dt[, l[1]]) /
        dtsd[, l[1]]
      cat("post dt\n")
    }
