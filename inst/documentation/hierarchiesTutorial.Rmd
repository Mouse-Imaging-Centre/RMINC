...
# some weirdness potentially particular to mac. Images are flipped, so fix that:
...
```{r}
# this takes the place of two calls:
# gf <- read.csv("input.csv")
# allvols <- anatGetAll(gf$labels)
load("~/Dropbox/data/scott-enrichment.RData")
...
labelVol <- mincArray(mincGetVolume("/Users/jason/Dropbox/data/atlases/Dorr_2008_Steadman_2013_Ullmann_2013_Richards_2011_Qiu_2016_Egan_2015_40micron/ex-vivo/DSURQE_40micron_labels.mnc"))
anatVol <- mincArray(mincGetVolume("/Users/jason/Dropbox/data/atlases/Dorr_2008_Steadman_2013_Ullmann_2013_Richards_2011_Qiu_2016_Egan_2015_40micron/ex-vivo/DSURQE_40micron_average.mnc"))
```
...
defs <- "~/Dropbox/colourbars/DSURQE_40micron_R_mapping.csv"
...
The main difference here is that rather than using the Benjamini & Hochberg (1995) method as is used for every other FDR invocation in RMINC, it uses the Benjamini and Yekutieli (2001) FDR algorithm, which is viable in the presence of dependence (as obviously exists in a hierarchy where lower level values are dependent on high levels).
