verboseRun({
  ...
  df <- read.csv(
    "/hpf/largeprojects/MICe/dfernandes/lqiu_long/all_relative_jacobians/files.csv"
  )
  ...
  test_that("MincLm works in parallel", {
    ...
    system.time(
      pMincLm <- mincLm(files ~ sex * age, data = df, parallel = c("local", 2))
    )
    system.time(
  ...
  anat_env <- new.env()
  test_inside(
    "AnatGetAll works in parallel",
    {
      anat_frame <-
        read.csv(
          "/hpf/largeprojects/MICe/vousdend/osmopumps/documents/final_osmostic_pump_image_list.csv"
        ) %>%
        filter(Useable == 1)

      anat_files <-
        system(
          "ls /hpf/largeprojects/MICe/vousdend/osmopumps/registration/full_maget/08feb15_maget/*_no_nuc_no_inorm_lsq6/labels/*votedlabels.mnc",
          intern = TRUE
        )
  ...
  test_that("VertexLmer Work in Parallel", {
    civet_example <- readLines(
      "/hpf/largeprojects/MICe/chammill/POND/civetOutputs/pond20151123/idfile"
    )
    thickness_df <-
      lapply(
        civet_example,
    ...
    system.time(
      vlmer <- vertexLmer(
        left ~ random_dose + (1 | random_group),
        data = thickness_df,
        mask = random_mask
      )
    )
